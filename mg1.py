class MG1:
    ...
    def __init__(self, arrival_rate, average_service_time, standard_dev):
        ...
        self.utilization = self.arrival_rate / self.average_service_rate
        ...
        self.queue_length = ((self.arrival_rate**2)*(self.standard_dev**2)+(self.utilization**2))/(2*(1-self.utilization))
        self.time_in_queue_name = 'Tiempo en la cola (Wq)'
        self.time_in_queue = self.queue_length / self.arrival_rate
        self.number_in_system_name =  "Cantidad de clientes en el sistema (L)"
        self.number_in_system =  self.utilization + self.queue_length
        self.time_in_system_name = 'Tiempo en el sistema (Wq)'
        self.time_in_system = self.average_service_time + self.time_in_queue

    def simulate(self):
        """ Simula la cola con los parámetros recibidos """
        return [
            self.arrivals_name + ': ' + str(round(self.arrival_rate, 3)),
            ...
            self.number_in_system_name + ': ' + str(round(self.number_in_system, 3)),
